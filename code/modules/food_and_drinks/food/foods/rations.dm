...
/obj/item/food/snacks/rations/nutrient_prism
	...
	desc = "The standard field ration for the USSP. An ultra-dense bar of compressed nutritional components and essential vitamins. \
	Difficult to chew. It has a shelf life so long that it has not yet been demonstrated to exist."
...
/obj/item/food/snacks/rations/nutrient_prism/attack(mob/M, mob/user)
	if(!opened)
		to_chat("<span class = 'warning'>[src] cannot be eaten without removing the packaging first.</span>")
		return
	to_chat("<span class = 'notice'>You start to bite into [src] and try to rip a small piece off.</span>")
	if(!do_after(3 SECONDS))	// It's VERY VERY chewy. A few months of service and you shall have the most strong, chiseled jaw ever. Like the soldiers on propagana posters have. Also balances out how insanely filling this is.
		to_chat("<span class = 'notice'>You give up on trying to bite through [src].</span>")
		return
	. = ..()
